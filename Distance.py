...
class DistanceFunction(nn.Module):
    def __init__(self, num_hidden,node_hidden,seq_length,dropout,time_emb_dim):
        super().__init__()
        # attributes
        self.hidden_dim = num_hidden
        self.node_dim   = node_hidden
        self.time_slot_emb_dim  = self.hidden_dim
        self.input_seq_len      = seq_length
        ...
        self.time_slot_embedding = nn.Linear(time_emb_dim, self.time_slot_emb_dim)
        # Distance Score
        self.all_feat_dim = self.ts_feat_dim + self.node_dim+ time_emb_dim*2
